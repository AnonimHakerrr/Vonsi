...
import { userData, bookings, rentals, skiPasses } from "../Data/mockData";

export default function DashboardPage() {
  const [activeTab, setActiveTab] = useState("overview");
  const [settingsModalOpen, setSettingsModalOpen] = useState(false);
  const [editableUserData, setEditableUserData] = useState(userData);
  const [bookingDetailsModal, setBookingDetailsModal] = useState<string | null>(
    null
  );
  ...
  const handleSaveSettings = () => {
    console.log("Saving user data:", editableUserData);
    setSettingsModalOpen(false);
  };

  const getBookingDetails = (id: string) => bookings.find((b) => b.id === id);
  const getRentalDetails = (id: string) => rentals.find((r) => r.id === id);
  const booking = bookingDetailsModal
    ? getBookingDetails(bookingDetailsModal)
    : null;
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6 mb-6">
            ...
            <Avatar className="h-16 w-16 sm:h-20 sm:w-20">
              <AvatarImage
                src={editableUserData.avatar || "/placeholder.svg"}
            ...
            <div className="flex flex-col items-center sm:items-start gap-1 sm:gap-2 text-center sm:text-left">
              ...
              <div className="flex flex-col sm:flex-col items-center sm:items-start gap-1 sm:gap-2 text-muted-foreground">
                <span className="text-sm sm:text-base">
                  Приєдналися {editableUserData.memberSince} року
                </span>
          ...
          <TabsContent value="overview" className="space-y-6 ">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Найпопулярніші функції для вашої зручності
                </CardDescription>
              </CardHeader>
              <CardContent>
          ...
          {booking ? (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                >
                  {booking.status === "confirmed" ? "Підтверджено" : "Активний"}
                </Badge>
              </div>

              {/* Номер готелю */}
              {booking.type === "Номер готелю" && booking.details && (
                    ...
                    <div>
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span>{booking.details.roomNumber}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Поверх:</span>
                          <span>{booking.details.floor}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Площа:</span>
                          <span>{booking.details.size}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Ліжко:</span>
                          <span>{booking.details.bedType}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span>{booking.details.totalNights}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span>{booking.details.pricePerNight}</span>
                        </div>
